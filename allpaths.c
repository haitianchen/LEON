...
char *debug_print_path(PlannerInfo *root, Path *path, int indent);
static char *debug_print_relids(PlannerInfo *root, Relids relids);
static char *print_rel_info(PlannerInfo *root, RelOptInfo *rel);
void get_calibrations(double calibration[], uint32 queryid, int32_t length);
/*
 * make_one_rel
...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
{
	...
	for (lev = 2; lev <= levels_needed; lev++)
	{
		...
		foreach (lc, root->join_rel_level[lev])
		{
			...
			if (ml_clf_enable)
			{
				ListCell *p;
				char *pathBuf;
				char *relBuf = print_rel_info(root, rel);
				appendStringInfo(&buf, relBuf);
				foreach (p, rel->pathlist)
				{
					Path *path = (Path *)lfirst(p);
					pathBuf = debug_print_path(root, path, 1);
		...
		char savePath[100];
		sprintf(savePath, "/data1/chenx/Postgres/logs/pathlist_%u.txt", 0);
		FILE *ffp = fopen(savePath, "w");
		if (ffp == NULL)
		{
			printf("Fail to open file! errno: %m\n");
			exit(0);
		}
		fprintf(ffp, buf.data);
		fclose(ffp);
		...
		if (ml_clf_enable)
		{
			...
			get_calibrations(calibrations, root->parse->queryId, length);
		...
		foreach (lc, root->join_rel_level[lev])
		{
			...
			// 	}
			// 	char savePath[100];
			// 	sprintf(savePath, "/data0/chenx/Postgres/logs/pathlist_%u.txt", root->parse->queryId);
			// 	FILE *ffp = fopen(savePath, "a");
			// 	if (ffp == NULL)
			// 	{
			// 		printf("Fail to open file! errno: %m\n");
			// 		exit(0);
			...
			// 	double calibrations[100];
			// 	get_calibrations(calibrations, root->parse->queryId);
			// }
	...
	/* Deirectly select the best one here */
	if (ml_clf_enable)
	{
		Path *Best_one;
		ListCell *p;
		foreach (p, rel->pathlist)
		{
			...
			cmp = compare_path_costs(Best_one, path, TOTAL_COST);
			if (cmp > 0 ||
				(cmp == 0 &&
				 compare_pathkeys(Best_one->pathkeys,
								  path->pathkeys) == PATHKEYS_BETTER2))
				Best_one = path;
		}
		rel->pathlist = NIL;
		rel->pathlist = lcons(Best_one, rel->pathlist);
	}
...
{
	// char command[500];
	// sprintf(command, "~/miniconda3/envs/torch_gpu/bin/python ~/codesForClassifier/parsePlanToNodes.py -p %u 2>&1 | tail -n 1", queryid);

	// char *command = "~/miniconda3/envs/torch_gpu/bin/python ~/codesForClassifier/pasePlanTesting.py -p 0 2>&1 | tail -n 1";
	char *command = "~/miniconda3/envs/torch_gpu/bin/python ~/codesForClassifier/parsePlanToNodes.py -p 0 2>&1 | tail -n 1";

	char buffer[128];
	/*always use calloc not malloc !(malloc + memset)*/
	char *result = (char *)calloc(10 * length, 1);
	...
	int32_t idx = 0;
	char *p = NULL;
	p = strtok(result, delim);

	calibration[0] = atof(p);
	idx++;
	while ((p = strtok(NULL, delim)))
	{
		calibration[idx] = atof(p);
		idx++;
	}
...
static char *
debug_print_relids(PlannerInfo *root, Relids relids)
{
	int x;
	bool first = true;
	StringInfoData buf;
...
char *
debug_print_path(PlannerInfo *root, Path *path, int indent)
{
	const char *ptype;
	bool join = false;
	Path *subpath = NULL;
	...
	if (path->parent)
	{
		char *RelidsBufPtr = NULL;
		appendStringInfoString(&buf, "(");
		RelidsBufPtr = debug_print_relids(root, path->parent->relids);
		appendStringInfoString(&buf, RelidsBufPtr);
		pfree(RelidsBufPtr);
		RelidsBufPtr = NULL;
	...
	if (path->param_info)
	{
		...
		RelidsBufPtr = debug_print_relids(root, path->param_info->ppi_req_outer);
		pfree(RelidsBufPtr);
		RelidsBufPtr = NULL;
		appendStringInfo(&buf, ")");
	}
	appendStringInfo(
		&buf, " rows=%.0f cost=%.2f..%.2f\n", path->rows, path->startup_cost, path->total_cost);
	// if (path->pathkeys)
	// {
	// 	for (i = 0; i < indent; i++)
	// 		printf("\t");
	...
	if (join)
	{
		...
		pathBufPtr = debug_print_path(root, jp->outerjoinpath, indent + 1);
		appendStringInfoString(&buf, pathBufPtr);
		pfree(pathBufPtr);
		pathBufPtr = NULL;

		pathBufPtr = debug_print_path(root, jp->innerjoinpath, indent + 1);
		appendStringInfoString(&buf, pathBufPtr);
		pfree(pathBufPtr);
		pathBufPtr = NULL;
	}
	...
	if (subpath)
	{
		pathBufPtr = debug_print_path(root, subpath, indent + 1);
		appendStringInfoString(&buf, pathBufPtr);
		pfree(pathBufPtr);
		pathBufPtr = NULL;
	}
...
static char *
print_rel_info(PlannerInfo *root, RelOptInfo *rel)
{
	StringInfoData buf;
	initStringInfo(&buf);
	char *RelidsBufPtr = NULL;
	...
	RelidsBufPtr = debug_print_relids(root, rel->relids);
	appendStringInfoString(&buf, RelidsBufPtr);
	pfree(RelidsBufPtr);
	RelidsBufPtr = NULL;
